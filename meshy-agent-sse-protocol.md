...
- 事件类型：不使用 SSE 的 `event:` 行，而由 JSON 内的 `type` 字段表示
- 普通文本：多个增量 JSON 事件拼成一条消息
...
一条文字消息由 `text-start`、零到多个 `text-delta`、`text-end` 组成：

```text
data: {"type":"text-start","thread_id":"thread_001","turn_id":"turn_001","step_id":"step_text_001","id":"msg_001"}
id: 1000-0

data: {"type":"text-delta","thread_id":"thread_001","turn_id":"turn_001","step_id":"step_text_001","id":"msg_001","delta":"这是一个"}
id: 1001-0

data: {"type":"text-delta","thread_id":"thread_001","turn_id":"turn_001","step_id":"step_text_001","id":"msg_001","delta":"可打印的 3D 模型。"}
id: 1002-0

data: {"type":"text-end","thread_id":"thread_001","turn_id":"turn_001","id":"msg_001"}
id: 1003-0
...
```ts
const messages = new Map<string, { content: string; streaming: boolean }>();

function handleEvent(event: AgentEvent & { delta?: string }) {
  switch (event.type) {
    case "text-start":
      messages.set(event.id!, { content: "", streaming: true });
      break;
...
收到这个事件后，前端应把卡片标记为已完成或折叠为 `Answered`。接着服务端继续下发新的 `text-start`、`text-delta`、`text-end` 事件。
...
表格作为普通 `text-delta` 的内容下发，并交由支持 GFM 的 Markdown renderer 显示：

```text
data: {"type":"text-start","id":"msg_table_001"}

data: {"type":"text-delta","id":"msg_table_001","delta":"| 方案 | 风格 | 预计打印时间 |"}

data: {"type":"text-delta","id":"msg_table_001","delta":"\n|---|---|---:|\n"}

data: {"type":"text-delta","id":"msg_table_001","delta":"| A | 忠于原画 | 3 小时 |\n"}

data: {"type":"text-delta","id":"msg_table_001","delta":"| B | 可爱玩具风 | 4 小时 |"}

data: {"type":"text-end","id":"msg_table_001"}
...
问答卡片：
  ...
  -> text-start / text-delta x N / text-end
